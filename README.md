...
- `apps/anvil`: a [Next.js](https://nextjs.org/) infra microservice for Anvil lifecycle + ADI deploy orchestration
...
#### Full stack wireup (recommended for local integration)

Starts services in order with health checks:
...
For production-mode startup after builds:

```sh
pnpm run start:wireup
```
